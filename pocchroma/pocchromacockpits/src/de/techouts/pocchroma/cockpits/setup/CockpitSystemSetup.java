 ...
 */
package de.techouts.pocchroma.cockpits.setup;
...
import de.techouts.pocchroma.cockpits.constants.PocchromaCockpitsConstants;
 ...
 */
@SystemSetup(extension = PocchromaCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(PocchromaCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(PocchromaCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						PocchromaCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/pocchromacockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
